...
- **Public web calendar** — opt-in per server (`/settings public-calendar on`, or the web Server settings page) and **off by default**: an unguessable link (`/c/<slug>`) to a login-free month calendar of the server's events, including upcoming occurrences of repeating ones; entries jump straight to the Discord message. It shows titles, times, locations, and channel names only — never descriptions, RSVPs, or member names — and the pages are `noindex` (shareable, not discoverable). `/settings public-calendar new-link` retires a leaked link; `off` closes it.

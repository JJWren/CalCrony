...
- **RSVPs** — one-click buttons (✅/❌/🤔 by default) with live-updating attendee lists on the embed. Creators can define their own buttons (any emoji + label, up to 10, e.g. `⚔️ Raider, 🛡️ Standby, ❌ Out`) with one option flagged as the "attending" one so roles, threads, and availability keep working; cap attendance (`attendee-limit:5` or a per-option capacity) and extra RSVPs queue on a **waitlist** — when a spot frees, the first in line is seated and pinged automatically; optionally **close RSVPs early** (`rsvp-close:"2h before"` or an absolute time) to freeze the attendee list before start; and allow **multiple RSVPs per member** (`multi-rsvp:true`, or the web form's checkbox) so one person can be Tank *and* Healer — clicking a chosen option again removes it, only the attending option keeps a waitlist, no option is exclusive (design choices that make sense together — with the default set a member could be Going *and* Not going), and turning it back off is refused while anyone holds more than one
...
- **Public web calendar** — opt-in per server (`/settings public-calendar on`, or the web Server settings page) and **off by default**: an unguessable link (`/c/<slug>`) to a login-free month calendar of the server's events, including upcoming occurrences of repeating ones; entries jump straight to the Discord message. It shows titles, start times, durations, locations, and channel names only — never descriptions, RSVPs, or member names — and the pages are `noindex` (shareable, not discoverable). `/settings public-calendar new-link` retires a leaked link; `off` closes it.
- **DM reminders** — strictly opt-in per user and **off by default** (`/settings dm-reminders enabled:true`, or My settings on the web): reminders and start announcements for events you're attending also arrive by DM, through the same crash-safe outbox. The bot offers the toggle exactly once, as an ephemeral prompt after your first attending RSVP — never an unsolicited DM — and if your DMs are closed to it, it switches the setting off instead of retrying. Servers and event creators can never turn it on for you.
...
- **Server activity log & CSV export** — every user-initiated management action (event create/edit/delete/skip, series schedule changes and stops, poll create/close/convert/delete, template save/edit/delete, server settings and public-calendar changes, live lists) is recorded with who did it and from where (Discord or web), kept for 90 days (`Retention__ActionLogDays`), and shown to server managers on the web app's **Activity** tab with action/member filters; the same tab exports every event with its RSVPs (one row per RSVP, custom options and waitlist status included) as a UTF-8 CSV. Both free for every server — no premium tier.
- **Attendee roles** — pick an existing role at `/create attendee-role:` and everyone who RSVPs "Going" gets it automatically (revoked when they switch away, un-RSVP, or the event ends/is deleted); great for event pings and temporary channel access. Roles can also be set **per RSVP option** by mentioning one inside `rsvp-options` — `rsvp-options:"🛡️ Tank x2 @tank, 💚 Healer x2 @healer, ⚔️ DPS x6 @dps"` sorts a raid into three roles from a single event, swapping a member's role over when they switch options (with multiple RSVPs on, a member seated on both holds both roles until their last seat carrying one goes). Requires the bot to hold **Manage Roles** with the picked roles below its own (on the current invite link; servers that added the bot earlier must re-invite or grant it to the bot's role).
- **Role-restricted signup** — `/create restrict-to:@Raiders` limits every RSVP option to members holding one of the named roles (up to 5), or restrict a single option inside `rsvp-options` with `only:` — `rsvp-options:"🛡️ Tank x2 @tank only: @raiders, ❌ Out"` grants `@tank` to whoever picks Tank and lets only `@raiders` pick it; `/poll create restrict-to:` does the same for voting. In Discord the bot checks your roles live; on the web the API answers from role snapshots the bot keeps and **fails closed** — if it can't confirm your roles (no snapshot yet, or the bot has been offline for more than 30 minutes) it asks you to RSVP from Discord rather than letting you through. The event creator and server managers always pass, a restriction only gates entry (losing the role later never costs a seat), a deleted role stops counting, and restrictions are set from Discord only (the web can remove one). See [ADR 0004](docs/adr/0004-role-restrictions-via-bot-role-snapshots.md).
...
|---|---|
| `/create title when [description duration channel location image repeat...]` | Create an event; `when` is natural language, `repeat` options make it recurring (`repeat:weekly repeat-days:tue,thu` or `repeat:weekly repeat-days:weekdays` for day sets — `repeat-days` always needs `repeat:weekly` — still `repeat-every` N weeks), `template` starts from a saved template, `attendee-role` grants a role to attending RSVPs, `thread` opens a discussion thread; `rsvp-options:"⚔️ Raider x10, ❌ Out"` customizes the buttons (first entry — or a trailing `*` — marks the attending option; `xN` caps it; `only: @role` after the entry limits who may pick it), `attendee-limit` caps attendance with a waitlist, `rsvp-close:"2h before"` stops RSVP changes early, `restrict-to:@role` limits every option to holders of the named roles, `multi-rsvp:true` lets each member pick more than one option (clicking a chosen option again removes it) |
...
| `/edit name [fields...] [scope]` / `/delete name` | Edit/delete by (partial) title — creator or server manager only; repeating events need `scope` (this occurrence / whole series); `attendee-role` / `clear-attendee-role` change or remove the granted role; `restrict-to` / `clear-restriction` change or remove the signup restriction (existing RSVPs stay); `rsvp-options` / `attendee-limit` / `rsvp-close` (plus their `clear-` flags) reshape the buttons, cap, and cutoff — same labels keep their RSVPs, and raising the cap seats the waitlist in order; `multi-rsvp` turns multiple RSVPs on or off (off is refused while anyone holds more than one) |
...
| `/poll create question options [single-vote anonymous allow-options closes restrict-to]` | Create a poll; `options` is comma-separated, `closes` is natural language, `restrict-to` limits voting to holders of the named roles |
...
| `/settings view` · `/settings timezone` · `/settings dm-reminders` · `/settings server-timezone` · `/settings default-channel` · `/settings native-events` · `/settings public-calendar on\|off\|new-link` | Personal & server timezone · opt in to DM reminders (off by default) · where web-created embeds post · mirror events to the server's Events tab · share (or retire) a login-free public calendar link |
